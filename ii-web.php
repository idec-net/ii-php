...
class IIWeb extends IIFrontend {
	...
	function __construct($echoareas, $tpldir, $onpage) {
		...
		$links=[
			'<a class="toplink" href="http://ii-net.tk/iidownload">Скачать</a>',
			'<a class="toplink" href="http://iinet.sexy">Что такое ii</a>',
	...
	function printMsg($message, $viewonly=false) {
		$ret=$viewonly ? "" : "<div class='message-container'>";
		...
		$ret.="</div>";
		if(!$viewonly) {
			$ret.="<a class='reply' href='?msgid=".$message['id']."&reply'>&gt;</a>";
			$ret.="</div>";
		}
		return $ret;
	}
...
$ii_reader=new IIWeb($echolist, "./iiweb-material", 20);
?>
